...
export default function TransformationShowcase() {
  ...
  const data: Record<"women" | "men", Transformation> = {
    women: {
      ...
      description: "Our signature correction service for dry, flat tones. Includes a deep botanical protein bond-builder, expert freehand color painting, and a bouncy multi-layer blowout styling.",
      beforeImg: "Dull, brassy, dehydrated strands",
      afterImg: "Seamless, golden-honey dimensional balayage",
      combinedImgUrl: "/src/assets/images/woman_transformation_1779552923773.png",
    ...
    men: {
      ...
      description: "A complete aesthetic rework designed to structure the natural jawline. Features a seamless warm skin fade, dapper pompadour styling, and a straight-razor steam-towel beard line up.",
      beforeImg: "Untamed beard, low-fade structure loss",
      afterImg: "Crisp taper, sharp beard symmetry",
      combinedImgUrl: "/src/assets/images/man_transformation_1779552943470.png",
  ...
  return (
    <section id="transformation_journey" className="py-16 sm:py-24 bg-secondary/30 border-y border-border-custom relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-10 sm:mb-16">
          ...
          <p className="text-xs sm:text-sm text-text-muted mt-3 font-light max-w-xl mx-auto leading-relaxed">
            Witness the real, high-contrast, premium transformations crafted daily in our salons. Beauty is a precise alignment of identity and artistry.
          </p>
            ...
            <div className="lg:col-span-7 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 px-1 pt-1 text-[11px] sm:text-[10px] font-mono tracking-wider text-text-muted">
                <div className="bg-surface/50 border border-border-custom/60 px-3.5 py-2 rounded-xl flex items-center gap-2">
                  <span className="text-text-primary font-bold shrink-0">[BEFORE]</span>
                  <span className="truncate">{active.beforeImg}</span>
                </div>
                <div className="bg-surface/50 border border-border-custom/60 px-3.5 py-2 rounded-xl flex items-center gap-2">
                  <span className="text-accent font-bold shrink-0">[AFTER]</span>
            ...
            <div className="lg:col-span-5 flex flex-col justify-center">
              <div className="bg-surface rounded-[24px] p-8 border border-border-custom shadow-sm relative">
                <div className="absolute -top-3 left-8 bg-accent/10 border border-accent/30 rounded-full px-3 py-1 flex items-center gap-1">
                  ...
                  <span className="text-[9px] font-mono tracking-[0.1em] text-accent font-bold uppercase">Tested Premium Work</span>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <span className="text-[10px] font-mono tracking-widest text-[#a8a199] block uppercase">COMPLETED BY:</span>
                    <span className="text-xs font-medium text-primary mt-1 block">{active.stylist}</span>
                  </div>

                  <div>
                    <span className="text-[10px] font-mono tracking-widest text-[#a8a199] block uppercase">SERVICES RENDERED:</span>
                    <span className="text-xs font-medium text-accent mt-1 block">{active.serviceCompleted}</span>
                  </div>

                  <div>
                    <span className="text-[10px] font-mono tracking-widest text-[#a8a199] block uppercase">CRAFT STEPS PERFORMED:</span>
                    <ul className="grid grid-cols-1 gap-2 mt-2">
                      {active.steps.map((step, idx) => (
                        <li key={idx} className="flex items-center gap-2 text-xs font-light text-text-primary">
